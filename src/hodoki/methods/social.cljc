(ns hodoki.methods.social
  "social.cljc — 解き — ELV Disassembly + Materials Recovery DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from hodoki's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "解き — ELV Disassembly + Materials Recovery が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over hodoki's own domain (End-of-life vehicle (ELV) disassembly + materials recovery Tier-B actor. Adopts mature OEM-run ELV facility methodology (depollution →…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "hodoki R0: live social posting is Council Lv6+ + operator + "
